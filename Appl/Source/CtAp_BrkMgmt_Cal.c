/*
 *************************************************************************
 *                                                                       *
 *                           ATOM GMBH                                   *
 *                        VCU SoftWare Team                              *
 *                                                                       *
 *                       All rights reserved                             *
 *                                                                       *
 *************************************************************************
 */
/*******************************************************************************
 *  FILE INFORMATION:
 *  Filename:           CtAp_BrkMgmt_Cal.c
 *  File Creation Date: 12-Sep-2025
 *  Model Name:         CtAp_BrkMgmt
 *  Model Version:      1.144
 *  Model Author:       Mike Ni - Fri Aug 16 09:59:38 2024
 *  Simulink Coder version         : 9.8 (R2022b) 13-May-2022
 *  C/C++ source code generated on : Fri Sep 12 09:13:06 2025
 *
 * Target selection: autosar.tlc
 * Embedded hardware selection: Infineon->TriCore
 * Code generation objectives: Unspecified
 * Validation result: Not run

 *  MODIFICATION HISTORY:
 *  Last Saved Modification:  Mike Ni - Wed Sep 10 14:51:20 2025
 *
 *
 *******************************************************************************/
#include "CtAp_BrkMgmt_Cal.h"
#include "rtwtypes.h"
#include "CtAp_BrkMgmt_types.h"

/*  GLOBAL DATA DEFINITIONS */

/* Exported data definition */

/* ConstVolatile memory section */
#define YDF_OVERLAY_START_SEC_VAR_CONST_8
#include "YDF_Overlay_MemMap.h"        /* PRQA S 5087 *//* MD_MSR_MemMap */

/* Definition for custom storage class: VolatileConst_AtomAutoSar */
volatile const real32_T ChassisMgmt_BrkCmpDecelReqOffTim_CFG = 100.0F;
                            /* Brake compensate deceleration request off time */
volatile const real32_T ChassisMgmt_BrkCmpDecelReqOnTim_CFG = 100.0F;
                             /* Brake compensate deceleration request on time */
volatile const real32_T ChassisMgmt_BrkCmpDecelReqTqOffTar_CFG = 0.0F;
                   /* Brake compensate deceleration request off torque target */
volatile const real32_T ChassisMgmt_BrkCmpDecelReqTqOnTar_CFG = 0.0F;
                   /* Brake compensate deceleration request off torque target */
volatile const uint16_T ChassisMgmt_IBSRbsDesTqOverFltEnaTim_CFG = 1500U;
                           /* RBS desired torque over limit fault enable time */
volatile const uint16_T ChassisMgmt_IBSRbsDesTqOverFltMatureTim_CFG = 400U;
                           /* RBS desired torque over limit fault mature time */
volatile const real32_T ChassisMgmt_IBSRbsDesTqOverFltThd_CFG = 1.0F;/* IBS rotque over fault threshold */

#define YDF_OVERLAY_STOP_SEC_VAR_CONST_8
#include "YDF_Overlay_MemMap.h"        /* PRQA S 5087 *//* MD_MSR_MemMap */

/*
 * File trailer for generated code.
 *
 * [EOF]
 */
