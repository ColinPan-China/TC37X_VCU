        ...
        P2VAR(uint16,       AUTOMATIC, AUTOMATIC)                        lenBytePtr)
{
  ...
  if (Eth_30_Tc3xx_IsTxBufProvisonPossible(ctrlIdx, lenBytePtr) == TRUE)                                                 /* SBSW_ETH_30_TC3XX_INT_FCT_PARAM_PTR_PASS */
  {
    ...
    else
    {
      ...
      Eth_30_Tc3xx_EnterExclusiveArea(DATA_MULTI_CORE); /* PRQA S 3109 */ /* MD_MSR_14.3 */
      ...
      Eth_30_Tc3xx_ExitExclusiveArea(DATA_MULTI_CORE); /* PRQA S 3109 */ /* MD_MSR_14.3 */
    }
  }
  ...
  P2VAR(Eth_RxStatusType, AUTOMATIC, AUTOMATIC) RxStatusPtr)
{
  ...
  /* ----- Development Error Checks ------------------------------------- */
#if (ETH_30_TC3XX_DEV_ERROR_DETECT == STD_ON)
  Eth_30_Tc3xx_RxQueueIdxCoreIdMapStartIdxOfEthCtrlType queueIdxCoreIdMapStartIdx =
          Eth_30_Tc3xx_GetRxQueueIdxCoreIdMapStartIdxOfEthCtrl(localCtrlIdx);
  /* #10 Check plausibility of input parameters and initialization state */
  if (localCtrlIdx >= Eth_30_Tc3xx_GetSizeOfEthCtrl())
  {
    errorId = ETH_30_TC3XX_E_INV_CTRL_IDX;
  ...
  else if (Eth_30_Tc3xx_GetRxQueueIdxCoreIdMap(queueIdxCoreIdMapStartIdx + FifoIdx) != GetCoreID())
  {
    errorId = ETH_30_TC3XX_E_INV_PARAM;
  }
  else
...
} /* Eth_30_Tc3xx_ReceiveFifo */ /* PRQA S 6050, 6080 */ /* MD_MSR_STPAR, MD_MSR_STMIF */
  ...
  uint8 FifoIdx)
{
  ...
  /* No explicit check is added to prevent the below DET check when VLAN-ID based frame queuing is disabled because this
   * function is called only when the VLAN-ID based frame queuing is enabled */
  else if (Eth_30_Tc3xx_GetTxQueueIdxCoreIdMap(queueIdxCoreIdMapStartIdx + FifoIdx) != GetCoreID())
  {
    errorId = ETH_30_TC3XX_E_INV_PARAM;
  }
  else
