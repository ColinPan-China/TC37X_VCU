...
COM_LOCAL_INLINE FUNC(void, COM_CODE) Com_SetRxPduGrpStateToStop(Com_IpduGroupIdType subIpduGroupId)
{
  ...
  if(Com_IsRxPduInfoIndUsedOfIPduGroupInfo(subIpduGroupId))     /* COV_COM_RXPDUINFO_OF_IPDUGROUPINFO_TX */
  {
...
COM_LOCAL_INLINE FUNC(void, COM_CODE) Com_SetTxPduGrpStateToStop(Com_IpduGroupIdType subIpduGroupId)
{
  ...
  if(Com_IsTxPduInfoIndUsedOfIPduGroupInfo(subIpduGroupId)) /* COV_COM_RXPDUINFO_OF_IPDUGROUPINFO_TX */
  {
 ...
 *
**********************************************************************************************************************/
FUNC(void, COM_CODE) Com_ReceptionDMControl_SetDmState(Com_RxPduInfoIterType rxPduId, Com_RxPduDmStateType bRequestedActiveState)
{
  Com_RxTOutInfoIndStartIdxOfRxPduInfoType idxRxTOutInfoInd = Com_GetRxTOutInfoIndStartIdxOfRxPduInfo(rxPduId);
...
FUNC(void, COM_CODE) Com_IpduGroupControl_UpdateRxComIPduState(Com_IpduGroupVector ipduGroupVector, boolean initialize) /* PRQA S 3673, 3677 */ /* MD_MSR_Rule8.13, MD_Com_3677 */
{
  ...
  Com_PartitionsActivatableRxComIPdusStructIterType activatablePartitionsIdx;
  ...
  {
    Com_RxPduInfoIterType rxPduId = Com_GetRxPduInfoIdxOfActivatableRxComIPdus(idx);

    /* Is only working if only one TxIPduGroupISRLockCounter is present and thus only one partition */
    Com_PartitionsActivatableRxComIPdusStructIdxOfActivatableRxComIPdusType partitionIdx = Com_GetPartitionsActivatableRxComIPdusStructIdxOfActivatableRxComIPdus(idx);
    Com_RxIPduGroupISRLockCounterIdxOfPartitionsActivatableRxComIPdusStructType rxIpduGroupISRLockCounterIdx = Com_GetRxIPduGroupISRLockCounterIdxOfPartitionsActivatableRxComIPdusStruct(partitionIdx);

    /* Evaluate requested active state of the I-PDU */
    boolean bRequestedActiveState = FALSE;
    Com_PduGrpVectorEndIdxOfRxPduInfoType pduGrpVectorIdx = Com_GetPduGrpVectorStartIdxOfRxPduInfo(rxPduId);
    {
      /* #20 Iterate over all related ComIPduGroups of the current ComRxPdu */
      uint8_least iByteIndex = 0;
      for(; (pduGrpVectorIdx < Com_GetPduGrpVectorEndIdxOfRxPduInfo(rxPduId)) && (bRequestedActiveState == FALSE); pduGrpVectorIdx++)   /* COV_COM_IPDU_GROUP_STATE_ACTIVE */
      {
    ...
    if((Com_IsRxPduGrpActive(rxPduId)) != bRequestedActiveState)
    {
      /* COM613 Second, the function Com_IpduGroupControl shall start or stop all I-PDUs that change theire activation state because of this call of Com_IpduGroupControl respectively. */
  ...
  /* Is only working if only one TxIPduGroupISRLockCounter is present and thus only one partition */
  for(activatablePartitionsIdx = 0; activatablePartitionsIdx < Com_GetSizeOfPartitionsActivatableRxComIPdusStruct(); activatablePartitionsIdx++)
  {
    Com_RxIPduGroupISRLockCounterIdxOfPartitionsActivatableRxComIPdusStructType rxIpduGroupISRLockCounterIdx =
      Com_GetRxIPduGroupISRLockCounterIdxOfPartitionsActivatableRxComIPdusStruct(activatablePartitionsIdx);
    Com_IPduGroupHdlr_ReloadRxIPduGroupISRLockCounter(rxIpduGroupISRLockCounterIdx);
  }
  Com_RxIPduGroup_ExitExclusiveArea();
}
 ...
 *
**********************************************************************************************************************/
FUNC(Com_RxPduDmStateType, COM_CODE) Com_ReceptionDMControl_EvaluateActiveState(Com_IpduGroupVector ipduGroupVector, Com_RxPduInfoIterType rxPduId)     /* PRQA S 3673, 3677 */ /* MD_MSR_Rule8.13, MD_Com_3677 */
{
  ...
  uint8_least iByteIndex = 0;
  for(; ((pduGrpVectorIdx < Com_GetPduGrpVectorEndIdxOfRxPduInfo(rxPduId)) && (bRequestedActiveState == COM_INACTIVE_RXPDUDMSTATE)); pduGrpVectorIdx++) /* COV_COM_IPDU_GROUP_STATE_ACTIVE */
  {
    if((ipduGroupVector[iByteIndex] & Com_GetPduGrpVector(pduGrpVectorIdx)) != 0u)
    {
      /* An I-PDU is active if and only if at least one I-PDU group is active it belongs to */
...
/* Justifications for module-specific MISRA deviations:
   ...
   MD_Com_2843_rxPduId: MISRA2012-Rule18.1
     Reason:     The code is entirely safe, further investigation: rxPduId is always valid because it is a qualified CSL03-use-case Com_GetRxPduInfoInd.
								 The validity of RxPduInfoInd is checked with Com_IsRxPduInfoIndUsedOfIPduGroupInfo.
     Risk:       No risk.
     Prevention: No prevention necessary.
*/
