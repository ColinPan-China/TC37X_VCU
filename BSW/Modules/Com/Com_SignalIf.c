...
/*! \brief        This method updates the signal or group signal object identified by SignalId
    ...
    \context      TASK|ISR2
**********************************************************************************************************************/
COM_LOCAL_INLINE FUNC(void, COM_CODE) Com_SendSignal_WriteSignal(Com_TxSigInfoIterType idxTxSigInfo, P2CONST(void, AUTOMATIC, COM_APPL_DATA) SignalDataPtr, uint16 Length,
                                                                 Com_TxPduInfoIterType idxTxPduInfo);
#endif
 ...
 *                state.
    ...
    \context      TASK|ISR2
**********************************************************************************************************************/
COM_LOCAL_INLINE FUNC(void, COM_CODE) Com_SendSignal_CheckFilter(Com_TxSigInfoIterType idxTxSigInfo, P2CONST(void, AUTOMATIC, AUTOMATIC) newValuePtr, Com_TxPduInfoIterType idxTxPduInfo);
#endif
 ...
 *
**********************************************************************************************************************/
COM_LOCAL_INLINE FUNC(void, COM_CODE) Com_SendSignal_WriteSignal(Com_TxSigInfoIterType idxTxSigInfo, P2CONST(void, AUTOMATIC, COM_APPL_DATA) SignalDataPtr, uint16 Length, Com_TxPduInfoIterType idxTxPduInfo)  /* PRQA S 3206 */ /* MD_Com_3206 */
{
  if(idxTxSigInfo < Com_GetSizeOfTxSigInfo())   /* COV_COM_MISRA */
  {
# if (COM_TXSIGGRPINFOUSEDOFTXSIGINFO == STD_ON)
    /* #10 If the passed signal is a ComGroupSignal */
    if(Com_IsTxSigGrpInfoUsedOfTxSigInfo(idxTxSigInfo)) /* PRQA S 2991, 2995 */ /* MD_Com_2991, MD_Com_2995 */ /* COV_COM_SIGGRP_CHECK */
    {
...
FUNC(uint8, COM_CODE) Com_SendSignal_Processing(Com_TxSigInfoIterType idxTxSigInfo, P2CONST(void, AUTOMATIC, COM_APPL_DATA) SignalDataPtr, uint16 Length)
{
  ...
  Com_TxPduInfoIterType idxTxPduInfo = Com_GetTxPduInfoIdxOfTxSigInfo(idxTxSigInfo);
...
# if (COM_FILTERINFOUSEDOFTXSIGINFO == STD_ON)
    ...
    Com_SendSignal_CheckFilter(idxTxSigInfo, newValuePtr, idxTxPduInfo);        /* SBSW_COM_FCTCALL_WITH_P2CONST */
# endif
    /* #40 Write the passed signal data */
    Com_SendSignal_WriteSignal(idxTxSigInfo, SignalDataPtr, Length, idxTxPduInfo);      /* SBSW_COM_FCTCALL_WITH_P2CONST */

    /* #50 If the Tx ComIPdu is active, evaluate the trigger conditions, otherwise return COM_SERVICE_NOT_AVAILABLE */
    if(Com_IsTxPduGrpActive(idxTxPduInfo))      /* COV_COM_NO_IPDUGROUPS */
    {
# if ((COM_ONCHANGEUSEDOFTXSIGINFO == STD_ON) || (COM_TRIGGEREDOFTXSIGINFO == STD_ON))
      Com_SendSignal_EvaluateTriggerConditionAndRequestTransmit(idxTxSigInfo, newValuePtr, idxTxPduInfo);       /* SBSW_COM_FCTCALL_WITH_P2CONST */
# endif
    }
    else
    {
