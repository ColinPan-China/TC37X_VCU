...
# if (COM_RXDEFERREDEVENTCACHE == STD_ON)
    ...
    Com_SetRxNextEventCacheEntry(Com_GetRxNextEventCacheEntryIdxOfMainFunctionRxStruct(mainFunctionId), 0);     /* SBSW_COM_CSL_VAR_ACCESS */
# endif
...
# if (COM_RXACCESSINFOINDENDIDXOFRXPDUINFO == STD_ON)
  {
    ...
    Com_RxAccessInfoIndEndIdxOfRxPduInfoType com_RxAccessInfoIndIdx = Com_GetRxAccessInfoIndStartIdxOfRxPduInfo(rxPduId);
...
#  if (COM_GWEVENTCACHE == STD_ON)
    ...
    if(Com_IsGwTxPduDescriptionInfoUsedOfRxPduInfo(rxPduId))    /* COV_COM_FEATURE_ALWAYS_TRUE_TX */
    {
...
# if (COM_RXDEFERREDEVENTCACHE == STD_ON)
  ...
  if(Com_GetRxNextEventCacheEntry(nextEventCacheEntryIdx) != COM_NO_HANDLERXPDUDEFERREDIDXOFRXPDUINFO)
  {
    Com_RxPduInfoIterType idxRxPduInfo;
    fallbackRequired = FALSE;   /* Cache is not full. */

    /* #20 Iterate over all stored deferred events, and process them */
    while(Com_GetRxNextEventCacheEntry(nextEventCacheEntryIdx) > 0u)
    {
      Com_DecRxNextEventCacheEntry(nextEventCacheEntryIdx);     /* SBSW_COM_CSL_VAR_ACCESS */
      idxRxPduInfo = (Com_RxPduInfoIterType) Com_GetRxDeferredEventCache(Com_GetRxNextEventCacheEntry(nextEventCacheEntryIdx));
      if(Com_IsRxPduGrpActive(idxRxPduInfo))
      {
      ...
      if(Com_GetRxNextEventCacheEntry(nextEventCacheEntryIdx) == COM_NO_HANDLERXPDUDEFERREDIDXOFRXPDUINFO)      /* COV_COM_DEFERRED_EVENTCACHE_FALLBACK */
      {
