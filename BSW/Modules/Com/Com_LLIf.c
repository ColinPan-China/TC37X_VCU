...
# else
    CalloutResult = Com_GetTxPduCalloutFuncPtr(idx) ((PduIdType) ComTxPduId, (P2VAR(uint8, AUTOMATIC, AUTOMATIC)) PduInfo->SduDataPtr); /* SBSW_COM_FCT_PTR_CALL_CSL03 */
# endif
  }
...
COM_LOCAL_INLINE FUNC(void, COM_CODE) Com_RxIndication_CacheDeferredEvent(Com_RxPduInfoIterType comRxPduInfoIdx)
{
  ...
  if(Com_IsMainFunctionRxStructUsedOfRxPduInfo(comRxPduInfoIdx))        /* COV_COM_SILENT_TX */
  {
    ...
    if(Com_GetRxNextEventCacheEntry(nextEventCacheEntryIdx) < Com_GetRxDeferredEventCacheEndIdxOfMainFunctionRxStruct(mainFunctionRxIdx))
    {
      Com_SetRxDeferredEventCache(Com_GetRxNextEventCacheEntry(nextEventCacheEntryIdx), comRxPduInfoIdx);       /* SBSW_COM_ARRAY_WRITEACCESS */
      Com_IncRxNextEventCacheEntry(nextEventCacheEntryIdx);     /* SBSW_COM_CSL_VAR_ACCESS */
    }
    /* #20 Otherwise mark the cache as overflowed */
    else
    {
      Com_SetRxNextEventCacheEntry(nextEventCacheEntryIdx, COM_NO_HANDLERXPDUDEFERREDIDXOFRXPDUINFO);   /* SBSW_COM_CSL_VAR_ACCESS */
    }
  }
}
#endif
    ...
    if(Com_GetHandleRxDeferredGwDescription(Com_GetHandleRxDeferredGwDescriptionIdxOfRxPduInfo(ComPduId)) == 0u)
    {
      if(Com_IsMainFunctionRouteSignalsStructUsedOfRxPduInfo(ComPduId)) /* COV_COM_SILENT_TX */
      {
        ...
        if(Com_IsGwEventCacheUsedOfMainFunctionRouteSignalsStruct(mainFunctionRouteSignalsIdx)) /* COV_COM_GWEVENTCACHE_OF_MAINFUNCROUTE_USED_TX */
        {
          ...
          if(Com_GetGwEventCacheIdxOfGwEventCacheIndex(gwEventCacheIndexIdx) < Com_GetGwEventCacheEndIdxOfMainFunctionRouteSignalsStruct(mainFunctionRouteSignalsIdx))
          {
            /* #40 Store PDU Id in cache, increase gateway cache index */
            Com_SetGwEventCache(Com_GetGwEventCacheIdxOfGwEventCacheIndex(gwEventCacheIndexIdx), (PduIdType) ComPduId); /* SBSW_COM_GWCACHING */
...
#   else
      (void) Com_GetTxPduTTCalloutFuncPtr(idx) ((PduIdType) TxPduId, (P2VAR(uint8, AUTOMATIC, AUTOMATIC)) PduInfo.SduDataPtr);  /* SBSW_COM_FCT_PTR_CALL_CSL03 */
#   endif
    }
#  endif
...
#  else
      retVal = Com_GetRxPduCalloutFuncPtr(idx) ((PduIdType) rxPduId, (P2CONST(uint8, AUTOMATIC, COM_APPL_DATA)) PduInfoPtr->SduDataPtr);        /* SBSW_COM_FCT_PTR_CALL_CSL03 */
#  endif
    }
