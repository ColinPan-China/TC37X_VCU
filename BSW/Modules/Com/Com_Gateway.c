...
/*! \brief        This function initiates the transmission of txPdu on gateway description event
    ...
    \context      TASK|ISR2
 *********************************************************************************************************************/
COM_LOCAL_INLINE FUNC(void, COM_CODE) Com_RxProcessing_GwDescriptionTransmit(Com_TxPduInfoIterType ComTxPduId, boolean isImmediate, boolean withoutRepetitionFlag);
# endif
#endif
...
/*! \brief    This function exits the gw description routing exclusive area.
    ...
    \context  TASK
**********************************************************************************************************************/
COM_LOCAL_INLINE FUNC(void, COM_CODE) Com_GwDescription_ExitExclusiveArea(Com_GatewayDescriptionProcessingISRLockCounterIdxOfMainFunctionRouteSignalsStructType idx);
...
/*! \brief    This function checks if the ISR Lock must be opened
    ...
    \context  TASK
**********************************************************************************************************************/
COM_LOCAL_INLINE FUNC(void, COM_CODE) Com_GwDescription_ThresholdCheck(Com_GatewayDescriptionProcessingISRLockCounterIdxOfMainFunctionRouteSignalsStructType idx);
#endif
...
COM_LOCAL_INLINE FUNC(void, COM_CODE) Com_GwDescription_ThresholdCheck(Com_GatewayDescriptionProcessingISRLockCounterIdxOfMainFunctionRouteSignalsStructType idx)
{
  ...
  {
    Com_GwDescription_ExitExclusiveArea(Com_GetGatewayDescriptionProcessingISRLockCounterIdxOfMainFunctionRouteSignalsStruct(idx));
    Com_GwDescription_EnterExclusiveArea();
  }
}
#endif
 ...
 *
 **********************************************************************************************************************/
FUNC(void, COM_CODE) Com_RxProcessing_GwDescriptionEvent(Com_RxPduInfoIterType rxComPduId, P2CONST(PduInfoType, AUTOMATIC, COM_APPL_DATA) PduInfoPtr)
{
...
# if ((COM_DIRECTOFTXMODETRUE == STD_ON) || (COM_DIRECTOFTXMODEFALSE == STD_ON))
    ...
    if(sendTrigger == TRUE)
    {
      ...
      Com_RxProcessing_GwDescriptionTransmit(ComTxPduId, Com_IsGwDestinationDescriptionTypeImmediate(typeOfDescription), withoutRepetitionFlag);        /* COV_COM_IMMEDIATE_DESTINATION_DESCRIPTION */
    }
# endif
...
FUNC(void, COM_CODE) Com_SignalGw_InitRxIpdu(Com_RxPduInfoIterType rxPduId)
{
  ...
  Com_GwInfoEndIdxOfRxPduInfoType idxGwInfo = Com_GetGwInfoStartIdxOfRxPduInfo(rxPduId);
 ...
 *
**********************************************************************************************************************/
FUNC(void, COM_CODE) Com_MainFunction_GwDescription(Com_MainFunctionRouteSignalsStructIterType mainFunctionId)
{
  Com_GatewayDescriptionProcessingISRLockCounterIdxOfMainFunctionRouteSignalsStructType gwDescriptionProcessingISRLockCounterIdx =
    Com_GetGatewayDescriptionProcessingISRLockCounterIdxOfMainFunctionRouteSignalsStruct(mainFunctionId);

  /* #10 Iterate over all ComIPdus inside the cache */
  Com_GwDescription_EnterExclusiveArea();

  if(Com_IsGwEventCacheUsedOfMainFunctionRouteSignalsStruct(mainFunctionId))    /* COV_COM_GWEVENTCACHE_OF_MAINFUNCROUTE_USED_TX */
  {
    ...
    while(Com_GetGwEventCacheIdxOfGwEventCacheIndex(gwEventCacheIndexIdx) > 0u)
    {
      ...
      rxPdu = (Com_RxPduInfoIterType) Com_GetGwEventCache(Com_GetGwEventCacheIdxOfGwEventCacheIndex(gwEventCacheIndexIdx));
      ...
      Com_GwDescription_ThresholdCheck(gwDescriptionProcessingISRLockCounterIdx);
    }
  }
  Com_GwDescription_ExitExclusiveArea(gwDescriptionProcessingISRLockCounterIdx);
}
#endif
