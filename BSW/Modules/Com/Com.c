...
/**********************************************************************************************************************
  VERSION CHECK
**********************************************************************************************************************/
#if ((COM_SW_MAJOR_VERSION != 22u) || (COM_SW_MINOR_VERSION != 2u) || (COM_SW_PATCH_VERSION != 1u))
...
# if (COM_ACTIVATABLETXCOMIPDUS == STD_ON)
    {
      ...
      Com_PartitionsActivatableTxComIPdusStructIterType activatablePartitionsIdx;
      ...
      for(idx = 0; idx < Com_GetSizeOfActivatableTxComIPdus(); idx++)
      {
        ...
        Com_PartitionsActivatableTxComIPdusStructIdxOfActivatableTxComIPdusType partitionIdx = Com_GetPartitionsActivatableTxComIPdusStructIdxOfActivatableTxComIPdus(idx);
        Com_TxIPduGroupISRLockCounterIdxOfPartitionsActivatableTxComIPdusStructType txIpduGroupISRLockCounterIdx =
          Com_GetTxIPduGroupISRLockCounterIdxOfPartitionsActivatableTxComIPdusStruct(partitionIdx);
      ...
      /* Is only working if only one TxIPduGroupISRLockCounter is present and thus only one partition */
      for(activatablePartitionsIdx = 0; activatablePartitionsIdx < Com_GetSizeOfPartitionsActivatableTxComIPdusStruct(); activatablePartitionsIdx++)
      {
        Com_TxIPduGroupISRLockCounterIdxOfPartitionsActivatableTxComIPdusStructType txIpduGroupISRLockCounterIdx =
          Com_GetTxIPduGroupISRLockCounterIdxOfPartitionsActivatableTxComIPdusStruct(activatablePartitionsIdx);
    ...
    {
      {
        ...
        for(idx = 0; idx < Com_GetSizeOfActivatableRxComIPdus(); idx++)
        {
          ...
          /* Evaluate requested active state of the I-PDU */
          Com_RxPduDmStateType bRequestedActiveState = Com_ReceptionDMControl_EvaluateActiveState(ipduGroupVector, rxPduId);    /* SBSW_COM_PTR_API_VALID_PTR */
          Com_ReceptionDMControl_SetDmState(rxPduId, bRequestedActiveState);
        }
      }
    }
# endif
...
/* START_COVERAGE_JUSTIFICATION
  ...
  \ID COV_COM_RXPDUINFO_OF_IPDUGROUPINFO_TX
    \ACCEPT TX
    \REASON An entry in IPduGroupInfo will either refer to RxPduInfo or TxPduInfo. The Condition will always be true for rx only configs.
