  ...
  )
{
  ...
  else if (DestinationBuffer->BufferSize < Dem_RecordReader_0x1904_CalculateSize_SingleRecordCombinedEvent(ReadoutBufferId, SRecId, SnapshotHeaderBitMaskIndex))
             ...
             && (lRecordNumber <= DEM_RECORDREADER_SNAPSHOTDATARECORD_TIMESERIES_FAST_LAST) )
          {
            lpSourceBuffer = 
              Dem_Memory_TimeSeries_EntryManager_GetFastConstDataPtr(lTimeSeriesEntry,
                                                         lRecordNumber - DEM_RECORDREADER_SNAPSHOTDATARECORD_TIMESERIES_FAST_FIRST);      /* SBSW_DEM_POINTER_FORWARD_STACK */
          }
          else
# endif
          {
            lpSourceBuffer =
              Dem_Memory_TimeSeries_EntryManager_GetNormalConstDataPtr(lTimeSeriesEntry,
                                                        lRecordNumber - DEM_RECORDREADER_SNAPSHOTDATARECORD_TIMESERIES_NORMAL_FIRST); /* SBSW_DEM_POINTER_FORWARD_STACK */
          }
  ...
  )
{
  ...
  {
    if (DestinationBuffer->BufferSize < Dem_RecordReader_0x1904_GetEventTimeSeriesPowertrainUdsSize(lEventId))
    {
      /* supported record, but the buffer is not large enough for all the data */
      lReturnValue = DEM_BUFFER_TOO_SMALL;
    }
  ...
  )
{
  ...
  *SizeOfFreezeFrame = 0x00;                                                                                                     /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
  ...
  if (Dem_Memory_TimeSeries_TestEventSupportTimeSeries(EventId) == TRUE)
  {
    if (Dem_DTCReadoutBuffer_GetTimeSeriesEntryIndex(ReadoutBufferId) != DEM_MEM_INVALID_MEMORY_INDEX)
    {
      *SizeOfFreezeFrame = Dem_RecordReader_0x1904_GetEventTimeSeriesPowertrainUdsSize(EventId);                                 /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
    }
  ...
  )
{
  ...
  else
  {
    ...
    {
      /* Event has a stored OBD freeze frame */
      if ((((uint32)DestinationBuffer->WriteIndex) + Dem_Cfg_EventObdOnUdsFFUdsSize(lEventId)) > DestinationBuffer->BufferSize)
      {
        /* supported record, but the buffer is not large enough for the data */
        lReturnValue = DEM_BUFFER_TOO_SMALL;
      }
