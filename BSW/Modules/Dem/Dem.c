...
\ID SBSW_DEM_ARRAY_WRITE_MONITORACTIVITYNUMERATORINCREMENTFLAG
  \DESCRIPTION    In a function, data is wirtten to array Dem_Cfg_ObdIumprData.MonitorActivityNumeratorIncrementedFlag[] using a function argument as index.
...
\CM CM_DEM_M_SIZEOF_OBDIUMPRDATA
  ...
  verify that Dem_Cfg_ObdIumprData.MonitorActivityNumerator[] is defined for each index in range [0.. DEM_G_NUMBER_OF_EVENT_WITH_MAR[,
  verify that Dem_Cfg_ObdIumprData.MonitorActivityNumeratorIncrementedFlag[] is defined for each index in range [0.. DEM_G_NUMBER_OF_MONITOR_ACTIVITY_NUMERATOR_INCREMENTD_ETRIES[ and
  verify that Dem_Cfg_ObdIumprData.StoredMonitorActivityRatio[] is defined for each index in range [0.. DEM_G_NUMBER_OF_STORED_MONITOR_ACTIVITY_RATIO[ .
...
\CM CM_DEM_R_DEM_CFG_OBDIUMPRDATA_MONITORACTIVITYNUMERATORINCREMENTFLAG_INDEX_CHECK
  Verify that the array index is in range before writing toDem_Cfg_ObdIumprData.MonitorActivityNumeratorIncrementedFlag.
