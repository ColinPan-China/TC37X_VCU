  ...
  )
{
  ...
  Dem_Readiness_Completion_SetStaticallyCalculatedCompletionStatesPID(&lPid01Data, DEM_COMPLETION_PIDCONTEXT_PID01_AND_PIDF501); /* SBSW_DEM_POINTER_FORWARD_STACK */
...
{
#if ((DEM_CFG_SUPPORT_MAJOR_MONITORS == STD_ON) || (DEM_CFG_PROCESS_PID41_COMPREHENSIVE_COMPLETE == STD_ON))
  uint32 lStaticCompletionMask;
  if (PIDContext == DEM_COMPLETION_PIDCONTEXT_PID01_AND_PIDF501)
  {
    lStaticCompletionMask = 0xFF00FFFFu | ((uint32)((uint32)Dem_Cfg_GetGlobalReadinessGroupAlwaysCompletedMaskByteB() << 16u));
    *PidValue &= lStaticCompletionMask;                                                                                          /* SBSW_DEM_POINTER_WRITE_ARGUMENT */
  }
  else
  {
    lStaticCompletionMask = 0xFF00FFFFu | ((uint32)((uint32)Dem_Cfg_GetGlobalPID41AlwaysCompletedMaskByteB() << 16u));
    *PidValue &= lStaticCompletionMask;                                                                                          /* SBSW_DEM_POINTER_WRITE_ARGUMENT */
  }
#else
...
)
{
  ...
  DIDF501[1] = DIDF501[1] & Dem_Cfg_GetGlobalReadinessGroupAlwaysCompletedMaskByteB();                                           /* SBSW_DEM_WRITE_PIDF501_BUFFER */
#else
...
# if (DEM_CFG_PROCESS_PID41_COMPREHENSIVE_COMPLETE == STD_ON)
    || (ReadinessGroup == DEM_CFG_READINESS_CMPRCMPT) /* PIDContext == DEM_COMPLETION_PIDCONTEXT_PID41 is always true */
# endif
