...
)
{
  ...
  *SizeOfFilteredRecords = 0u;                                                                                                   /* SBSW_DEM_POINTER_WRITE_ARGUMENT */
       ...
       Dem_Dcm_ERec_ERecIter_Next(&lERecIter))                                                                                   /* SBSW_DEM_CALL_ITERATOR_POINTER */
  {
    ...
    {
      if (Dem_ERec_IsERecAvailableForEvent(lEventId, lERecId))
      {
        lERecCount++;
...
# if (DEM_CFG_SUPPORT_ERECS == STD_ON)
  {
    ...
    while (Dem_ExtendedEntry_IterExists(&lExtendedEntryIter) == TRUE)                                                            /* SBSW_DEM_CALL_ITERATOR_POINTER */
    {
      ...
      /* If the extended data number matches */
      if ((Dem_ERec_IsERecAvailableForEvent(lEventId, lCurrentRecordNumber) == TRUE)
        && (Dem_Dcm_ERec_TestRecordMatch(lReadoutBufferId, lCurrentRecordNumber) == TRUE) )                                      /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
      {
...
# if (DEM_CFG_SUPPORT_ERECS == STD_ON)
  {
    ...
    if (lRecordNumber < 0xf0U)
    {
      ...
      if (Dem_ERec_IsERecAvailableForEvent(lEventId, lRecordNumber) == TRUE)
      {
        Dem_Cfg_MemoryEntryHandleType lMemoryEntryId;
        uint8 lExtendedEntryIndex;
    ...
    else
    {
      ...
      if (lEventId != DEM_EVENT_INVALID)
      {
             ...
             Dem_ExtendedEntry_IterNext(&lExtendedEntryIter))                                                                    /* SBSW_DEM_CALL_ITERATOR_POINTER */
        {
          ...
          if ((Dem_ERec_IsERecAvailableForEvent(lEventId, lCurrentRecordNumber) == TRUE)
              && (lCurrentRecordNumber > lERecMinNumber) )
...
)
{
  ...
  if ((lSize + (uint8)Dem_Dcm_ERecSizeForERecId) <= (uint16)(DestinationBuffer->BufferSize - DestinationBuffer->WriteIndex))
  {
...
/* ****************************************************************************
* Dem_ERec_IsERecAvailableForEvent
*****************************************************************************/
/*!
* Internal comment removed.
 *
...
{
  /* Set return value to default: TRUE */
  boolean lReturnValue = TRUE;
  /* If OBD on UDS is configured in any variant of the current configuration */
  if ((Dem_Cfg_IsObdOnUdsSupported() == TRUE)
    && (RecordNumber == DEM_EREC_OBDONUDS_ERECNUM_MAR)
